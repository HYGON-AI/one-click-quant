...
def _is_moe_gate_module(name: str, mod: torch.nn.Module) -> bool:
    """匹配 Kimi/Qwen/GLM-5.3 MoE router/gate。"""
...
def _install_routing_hooks(model: torch.nn.Module):
    """挂 MoE gate/router forward hook，收集每层 topk_idx + topk_weight。

    返回 (hooks, routing_dict, debug_after_forward)：调用方负责在跑完 forward 后
    自行 remove hooks，再调用 debug_after_forward() 打印命中情况。这样可以把
    routing hooks 和 per-layer / submodule hooks 挂在同一次 forward 上，避免为
    --log-routing 单独多跑一遍完整 forward。
    """
    hooks: list = []
    routing: dict = {}
    gate_topk = _infer_model_topk(model)
    candidates = list(_iter_moe_gate_modules(model))
    # print(
    #     f"  [routing_debug] model={type(model).__name__}, "
    #     f"config={type(getattr(model, 'config', None)).__name__}, "
    #     f"gate_topk={gate_topk}, candidates={len(candidates)}"
    # )
    if candidates:
        for name, mod in candidates:
            # print(f"  [routing_debug] candidate: {name} ({type(mod).__name__})")
            pass
    else:
        for name, mod in model.named_modules():
            lname = name.lower()
            cls = type(mod).__name__.lower()
            if any(token in lname or token in cls for token in ("mlp", "moe", "gate", "router", "expert")):
                # print(f"  [routing_debug] related: {name or '<root>'} ({type(mod).__name__})")
                pass
    ...
    for name, mod in candidates:
        def _make_hook(n):
            def _hook(module, inp, out):
                hook_called.add(n)
                if len(hook_called) <= 3:
                    if torch.is_tensor(out):
                        out_desc = f"Tensor shape={tuple(out.shape)} dtype={out.dtype}"
                    elif isinstance(out, (tuple, list)):
                        parts = []
                        for item in out[:4]:
                            parts.append(
                                f"Tensor shape={tuple(item.shape)} dtype={item.dtype}"
                                if torch.is_tensor(item) else type(item).__name__
                            )
                        out_desc = f"{type(out).__name__}[{', '.join(parts)}]"
                    else:
                        out_desc = type(out).__name__
                    # print(f"  [routing_debug] hook called: {n}; output={out_desc}")
                normalized = _normalize_routing_output(module, out, gate_topk)
                if normalized is not None:
                    routing[n] = normalized
                    if len(routing) <= 3:
                        # print(
                        #     f"  [routing_debug] normalized: {n}; "
                        #     f"indices_shape={tuple(normalized[0].shape)}, "
                        #     f"weights_shape={tuple(normalized[1].shape)}"
                        # )
                        pass
                elif len(hook_called) <= 3:
                    # print(f"  [routing_debug] normalize returned None: {n}")
                    pass
            return _hook
        hooks.append(mod.register_forward_hook(_make_hook(name)))

    def _debug_after_forward():
        if candidates:
            # print(
            #     f"  [routing_debug] hook_called={len(hook_called)}/{len(candidates)}, "
            #     f"routing_records={len(routing)}"
            # )
            pass
        elif not routing:
            # print("  [routing_debug] 未发现候选 gate；请检查模型是否实际加载为 GLM-5.3 MoE")
            pass

    return hooks, routing, _debug_after_forward
...
def _run_and_collect(
    ...
    for input_ids, desc in cases:
        ...
        routing_dict: dict = {}
        hooks: list = []
        routing_debug_after = None

        if dump_per_layer:
            # 挂 hook 收集每层 DecoderLayer 输出
            for name, mod in model.named_modules():
        ...
        if log_routing:
            # routing hooks 与 per-layer / submodule hooks 共用同一次 forward，
            # 避免为 --log-routing 单独多跑一遍完整 forward。
            routing_hooks, routing_dict, routing_debug_after = _install_routing_hooks(model)
            hooks.extend(routing_hooks)
        ...
        try:
            ...
            for h in hooks:
                h.remove()
        if routing_debug_after is not None:
            routing_debug_after()

        layer_hs = hs_dict if dump_per_layer else None
        submodule_hs = sub_dict if dump_submodule else None
        routing = routing_dict if log_routing else None
        if log_routing and not routing:
            print("  [routing_debug] 本次 forward 未采集到路由记录")
...
def _compare_collected(
    ...
    for rec_a, rec_b in zip(records_a, records_b):
        ...
        if log_routing:
            ...
            elif routing_a is None or routing_b is None:
                print("  [routing] 未采集路由数据，跳过路由分析")
...
def _sequential_compare(
    ...
    def _run_in_subprocess(label: str, model_path: Path):
        ...
        # 用 timeout + is_alive 轮询，避免子进程被 OOM-killer / SIGKILL / segfault
        # 干掉（没机会 queue.put）时父进程永久挂在 q.get()。
...
def _log_routing_diff(routing_a: dict, routing_b: dict):
    ...
    if not keys:
        print(
            f"  [routing] 没有可比较的路由记录 "
            f"(model_a={len(routing_a)}, model_b={len(routing_b)})"
        )
        return
