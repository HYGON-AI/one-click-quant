...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
模型路径约定（build_mini_glm5_3.py 构建产物）
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  /model/GLM53-mini-orig-L{start}-{end}    # fp8-block dequant 后的 bf16 基线
  /model/GLM53-mini-w8a16-L{start}-{end}   # w8a16 packed, HF 加载走 ct_decompress

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
1. 单段模式 — 直接对比两个 mini 模型
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
# 基础对比（原始 vs w8a16，前 4 层）
python3 compare_mini_models.py \
    --model-a /model/GLM53-mini-orig-L0-4 \
    --model-b /model/GLM53-mini-w8a16-L0-4 \
    --cpu-convert-then-dispatch          # W8A16 packed 加载必须走这条
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
2. 级联模式 — 每段共享上一段 orig 输出的 hidden state
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
# 级联对比（3 段）
python3 compare_mini_models.py \
    --model-a /model/GLM53-mini-orig-L0-4,/model/GLM53-mini-orig-L4-8,/model/GLM53-mini-orig-L8-12 \
    --model-b /model/GLM53-mini-w8a16-L0-4,/model/GLM53-mini-w8a16-L4-8,/model/GLM53-mini-w8a16-L8-12 \
    --cascade --cpu-convert-then-dispatch --fix-routing --dump-per-layer

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
主要选项速查
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  --cpu-convert-then-dispatch  W8A16 packed 权重必须启用（CPU→decompress→GPU dispatch）
  ...
  --save-embeds-dir            级联时保存每段 orig hidden state 到目录
  --load-embeds-prefix         从 {prefix}_bs{bs}_len{seq}_hs.pt 自动加载初始 inputs_embeds
  --num-cases                  只跑前 N 条 test prompt（默认全部）
  --max-memory-per-gpu         每卡显存预算，如 120GiB
  --module-device-overrides    自定义模块设备映射，用于修正 tensor device 不一致
...
def _is_decoder_layer_module(mod: torch.nn.Module) -> bool:
    """匹配 GLM/Kimi/Qwen/Llama 等常见 DecoderLayer，供逐层 hidden state hook 使用。"""
...
def _is_moe_gate_module(name: str, mod: torch.nn.Module) -> bool:
    """匹配 GLM-5.3/Kimi/Qwen MoE router/gate。"""
    cls_name = type(mod).__name__
    cls = cls_name.lower()
    lname = name.lower()
    return (
        cls_name == "KimiMoEGate"
        or cls_name in {"GlmMoeDsaTopkRouter", "GlmMoeDsaRouter"}
        or lname.endswith(".mlp.gate")
        or lname.endswith(".moe.gate")
        or "router" in lname
        or "gate" in cls
        or "router" in cls
    )
...
def _iter_moe_gate_modules(model: torch.nn.Module):
    """枚举显式命名或挂在 MoE MLP 上的 gate，兼容 GLM-5.3。"""
    seen = set()
    for name, mod in model.named_modules():
        ...
        if isinstance(gate, torch.nn.Module) and id(gate) not in seen:
            module_name = f"{name}.gate" if name else "gate"
            if "mlp" in name.lower() or "moe" in type(mod).__name__.lower():
                seen.add(id(gate))
                yield module_name, gate
...
def _infer_gate_topk(module: torch.nn.Module, logits: torch.Tensor, top_k: int | None = None) -> int:
    ...
    # 只挑真正表示 top-k 的属性；n_routed_experts 是总专家数，不能当 top-k，
    # 否则 _normalize_routing_output 里 torch.topk 会返回全部 expert，
    # 导致 --log-routing 的重叠率永远 100%。
...
def _normalize_routing_output(module: torch.nn.Module, out, top_k: int | None = None):
    ...
    if isinstance(out, (tuple, list)) and len(out) >= 3:
        logits, weights, indices = out[0], out[1], out[2]
        if torch.is_tensor(indices) and torch.is_tensor(weights):
            if indices.dtype in (torch.int8, torch.int16, torch.int32, torch.int64):
...
def load_model(
    ...
    大 mini (含 W8A16/W4A16/MXFP4 quantization_config 且 checkpoint > 50GB) 解压后可能到
    数百 GB, 单卡装不下。当 device="auto" 时采用两阶段策略:
      ...
      - CPU RAM 需要同时容纳 packed/dense 过渡期间的峰值
    """
    ...
    if device == "cpu":
        ...
        if need_offload and device != "cpu":
            ...
            # ── 最终 dispatch 的真实显存预算 ──
            # 若本进程已加载了 model_a, GPU 上已有它的占用。infer_auto_device_map
            # 只按传入的 max_memory 分配, 不查真实 free, 所以必须用 mem_get_info
            # 把已占用扣掉; 否则第二个模型 dispatch 时 `.to(cuda:i)` 会因累计
            # 超过物理显存而 OOM (即便静态 max_memory 名义上高于实际空闲)。
            def _parse_mem_gib(spec: str) -> float:
                s = spec.strip()
                ...
                return float(s) / 1024 ** 3  # 裸数字视为 bytes

            n_gpu = torch.cuda.device_count()
            # 释放本进程未使用的 caching block, 让 mem_get_info 汇报接近真实空闲
            for _i in range(n_gpu):
                with torch.cuda.device(_i):
                    torch.cuda.empty_cache()

            _DISPATCH_HEADROOM_GIB = 6.0  # 留给 .to() 拷贝 / accelerate hook 的余量
            dispatch_max_memory = dict(max_memory) if max_memory is not None else {}
            free_report: list[tuple[float, float]] = []
            for i in range(n_gpu):
                total_gib = torch.cuda.get_device_properties(i).total_memory / 1024 ** 3
                if max_memory is not None:
                    static_cap_gib = _parse_mem_gib(max_memory[i])
                else:
                    static_cap_gib = max(1.0, min(total_gib * 0.90, total_gib - 16))
                free_bytes, _ = torch.cuda.mem_get_info(i)
                free_gib = free_bytes / 1024 ** 3
                free_report.append((free_gib, total_gib))
                cap_gib = max(1.0, min(static_cap_gib, free_gib - _DISPATCH_HEADROOM_GIB))
                dispatch_max_memory[i] = f"{int(cap_gib)}GiB"
            dispatch_max_memory.setdefault("cpu", "1800GiB")
            print(
                "  [offload] per-GPU free/total GiB: "
                + ", ".join(f"{i}:{f:.1f}/{t:.1f}" for i, (f, t) in enumerate(free_report))
            )
            ...
            if hasattr(model, "ct_decompress_hook"):
                print("  [offload] step 2: CPU 上主动 decompress packed → BF16 (W8A16/W4A16/...)...")
...
_REAL_PROMPTS = [
    ...
    "GLM-5.3 uses MoE architecture with routing gate, activated conditionally per token.",
...
def _get_compare_tensor(
    ...
    # 自定义 forward 不支持 output_hidden_states → 用 hook 捕获 lm_head 输入
...
def _collect_routing(
    ...
    for name, mod in _iter_moe_gate_modules(model):
        def _make_hook(n):
            def _hook(module, inp, out):
...
def _apply_fixed_routing(model: torch.nn.Module, fixed_routing: dict) -> list:
    ...
    Kimi gate 通常返回 (topk_idx, topk_weight)，可完整覆盖；GLM/Qwen gate 常见返回
    ...
    for name, mod in _iter_moe_gate_modules(model):
        ...
        def _make_hook(fixed_idx, fixed_w):
            def _hook(module, inp, out):
                ...
                if torch.is_tensor(out) and out.dim() >= 2:
                    # Qwen/GLM gate 若输出 logits，则用稀疏 logits 强制 top-k expert。
...
def _get_compare_tensor_fixed_routing(
    ...
    for name, mod in _iter_moe_gate_modules(model_a):
        def _make_c(n):
        ...
        while True:
            try:
                ...
                if not p.is_alive():
                    p.join()
                    raise RuntimeError(
                        f"{label} 子进程已退出 (exitcode={p.exitcode}) 但未上报结果 "
                        f"(可能被 OOM-killer / SIGKILL / segfault 干掉)"
                    )
        p.join()
        if p.exitcode != 0:
...
def main():
    ...
    parser.add_argument("--max-memory-per-gpu", type=str, default=None,
                        help="传给 from_pretrained/max_memory 的每卡显存预算，例如 120GiB")
    parser.add_argument("--cpu-convert-then-dispatch", action="store_true",
                        help="先在 CPU 完成 checkpoint 反量化，再 dispatch 到 GPU；加载 W8A16/W4A16 packed 权重必须启用")
    parser.add_argument("--module-device-overrides", type=str, default=None,
                        help="逗号分隔的 完整模块名=设备 映射；在自定义 dispatch 前覆盖模块设备，例如 module_a=cuda:0,module_b=cpu。设备可用 last-layer 表示最后一个 decoder 层所在设备")
    ...
    if args.cascade:
        ...
        cases = build_prompt_cases(tokenizer, exec_device, args.use_random, vocab_size, num_cases=args.num_cases)
        for input_ids, desc in cases:
    ...
    for path, label in [(model_a_path, "模型 A"), (model_b_path, "模型 B")]:
        if not path.exists():
            ...
            print("   请先运行 build_mini_glm5_3.py 构建 mini 模型。")
            sys.exit(1)
